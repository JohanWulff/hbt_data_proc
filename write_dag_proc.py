...
def return_executable(year,
    ...
    function_call_str = f'python ${{EXE}} -f $1 -m {model_name}'
    if parametrised:
        function_call_str += " -p"
    if combined:
        function_call_str += " --combined"
    file_str = f'#!/usr/bin/bash\n\
EXE="/eos/user/j/jowulff/res_HH/hbt_resonant_evaluation/evaluate_procfile.py"\n\
...
def main(submit_base_dir: str, 
         ...
         parametrised: bool,
         combined: bool):
    skims_dir=f"/eos/user/j/jowulff/res_HH/Condor_out/dnn_data_{year}"
...
files for sample ({i+1}/{len(samples)})\r", end="")
        ...
        if not os.path.exists(submitfile):
            submit_string = return_subfile(executable=afs_exe,)
